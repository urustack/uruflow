...
<p align="center">
<a >
  <img src="https://img.shields.io/badge/release-v1.0.1-3B82F6?style=flat-square" alt="release">
</a>
  <a>
...
- **real-time logs** — stream deployment output as it happens. not polling. actual streaming.
...
**agent** — runs on each deployment target, executes commands, streams logs, reports metrics.
...
[Service]
Type=simple
ExecStart=/usr/local/bin/uruflow
Restart=always
RestartSec=5
...
sudo systemctl enable uruflow        # or uruflow-agent
sudo systemctl start uruflow         # or uruflow-agent
```
